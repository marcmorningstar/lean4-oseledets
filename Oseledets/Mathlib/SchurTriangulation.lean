...
Vendored from `leanprover-community/physlib` (`Mathematics/SchurTriangulation.lean`, Gordon Hsu,
Apache-2.0). Delint-only adaptation for the strict `Oseledets` library; delete when Mathlib gains
Schur triangulation.

## Main definitions

- `Matrix.schur_triangulation` : a matrix `A : Matrix n n 𝕜` with `𝕜` being algebraically closed
can be decomposed as `A = U * T * star U` where `U` is unitary and `T` is upper triangular.
...
-- Schur's well-founded recursive triangulation elaborates the entire eigenspace decomposition,
-- the collected orthonormal basis, and the block upper-triangularity proof in a single term, so
-- it needs a raised heartbeat budget.
set_option maxHeartbeats 800000 in
-- The same recursive term also exceeds the default recursion depth during elaboration, so the
-- recursion-depth budget is raised in tandem.
